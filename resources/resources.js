...
import { fetch_json, splitN } from "./util.js";
...
function fetch_resources(url, bookmark_list, controller = undefined) {
	...
	if(url_path.startsWith("/bookmark/")) {
		...
		if(info.num === null) {
			let tag_id = bookmark_list.find_tag(info.tag);
			let items = bookmark_list.items(tag_id);
			return items.map(v => ({
...
export { fetch_resources, fetch_resource_path, update_links }
